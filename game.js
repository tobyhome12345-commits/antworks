/* =========================================================================
   ...
   SECTION MAP (all in this file; pull apart into modules later if it grows):
      ...
      9. PARTICLES     little dirt puffs
     10. UPDATE        per-frame simulation
     11. RENDER        tiles, ant, dig overlay, underground light, HUD
...
const CFG = {
  TILE: 32,              // px per tile
  MAP_W: 220,            // tiles wide  (spec: >= 200)
  MAP_H: 110,            // tiles tall  (spec: >= 100)
...
// Turn a small disc of dirt/rock into open tunnel.
...
// Does the ant's box, placed at (px,py), overlap any solid tile?
...
function movePlayer(dt) {
  ...
  // depth below the local ground line — EXTEND: drive an oxygen/light meter from this
...
window.addEventListener('blur', () => keys.clear());   // don't get stuck "held" on tab-out
...
function updateDigging(dt) {
  ...
  if (worked >= need) {
    ...
    setTile(tx, ty, T.TUNNEL);           // dug tiles stay dug — never regenerated
...
function update(dt) {
  ...
  updateParticles(dt);
  updateCamera(dt);
  // EXTEND: entity AI, resource spawns, oxygen drain, day/night, etc. go here.
}
...
// Cheap per-tile brightness jitter so flat fills don't look dead.
...
function render() {
  ...
  drawLighting();   // screen-space: darkness closes in the deeper you go
  drawHUD();        // screen-space: controls + stats
}
...
function drawTiles() {
  ...
  for (let ty = y0; ty <= y1; ty++) {
    for (let tx = x0; tx <= x1; tx++) {
      ...
      switch (v) {
        case T.AIR:
          break; // sky already painted

        case T.TUNNEL:
          ctx.fillStyle = rgb(42 + n, 31 + n, 25 + n);
          ctx.fillRect(px, py, S, S);
          ctx.fillStyle = 'rgba(0,0,0,0.25)';       // soft top shadow for depth
          ctx.fillRect(px, py, S, 3);
          break;

        case T.GRASS:
          ctx.fillStyle = rgb(120 + n, 78 + n, 46 + n); // dirt body
          ctx.fillRect(px, py, S, S);
          ctx.fillStyle = rgb(78 + n, 138 + n, 62 + n); // green cap
        ...
        case T.DIRT:
          ...
          ctx.fillRect(px, py, S, S);
          ctx.fillStyle = rgb(95 + n, 60 + n, 34 + n); // a couple of pebbly specks
...
function drawDigOverlay() {
  ...
  // partially-dug tiles: a shrinking dark core + a radial progress ring
  ...
  // outline the tile we're currently aimed at
  if (digTarget) {
...
function drawAnt() {
  ...
  // direction pip on the head
  ctx.fillStyle = '#ffcf6b';
  ctx.fillRect(10, -1.5, 3, 3);

  // a clod of carried dirt on the back
  if (dirtCarried > 0) {
...
// Underground gloom: a soft lit disc around the ant, darkness beyond it,
...
function frame(ts) {
  ...
  let dt = (ts - last) / 1000;
  last = ts;
  if (dt > 0.05) dt = 0.05;          // clamp big hitches (e.g. returning from another tab)
...
function init() {
  ...
  // place the camera on the ant immediately so frame 1 doesn't lurch
  cam.zoom = 1;
